...
def getRaja(aika):
    aika = aika + timedelta(hours=23)
    daattor = aika.strptime(date.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%fZ"), "%Y-%m-%dT%H:%M:%S.%fZ")
    daattor_vali = daattor.replace(tzinfo=ZoneInfo('UTC'))
    daattor_oikea = daattor_vali.astimezone(ZoneInfo('Europe/Helsinki'))
    return daattor_oikea
    ...
    try:
        while(running):
            ...
            for i in y['prices']:
                ...
                alaerotus = alinhinta_t - nyt_oikea
                try:
                    if(alinhinta_t > nyt_oikea and alaerotus.days == 0):
                        sekohinta[1].append(timeFinder(i['startDate']))
                except Exception as e:
                    print(e)
                ylaerotus = ylinhinta_t - nyt_oikea
                try:
                    if(ylinhinta_t > nyt_oikea and ylaerotus.days == 0):
